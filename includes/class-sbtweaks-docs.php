...
class SBTWEAKS_Docs {
	...
	public static function boot() {
		add_action( 'admin_post_sbtweaks_save_docs', [ __CLASS__, 'save' ] );
	}

	private static function path() {
		return SBTWEAKS_PATH . 'docs/context.md';
	}
	...
	public static function save() {
		...
		check_admin_referer( 'sbtweaks_save_docs' );

		$text = isset( $_POST['sbtweaks_docs'] ) ? (string) wp_unslash( $_POST['sbtweaks_docs'] ) : '';
		$file = self::path();
	...
	public static function render() {
		...
		echo '<section class=' . $q . 'sbtweaks-section' . $q . '>';
		echo '<div class=' . $q . 'sbtweaks-section__head' . $q . '><h2>' . esc_html__( 'Notes for next time', 'sb-tweaks' ) . '</h2>';
		...
		echo '<div class=' . $q . 'sbtweaks-section__body' . $q . '>';
		...
		echo '<h3 class=' . $q . 'sbtweaks-docs__heading' . $q . '>' . esc_html__( 'Starting a new chat', 'sb-tweaks' ) . '</h3>';
		echo '<p class=' . $q . 'description' . $q . '>' . esc_html__( 'Copy this in as the first message, so the chat knows where to look.', 'sb-tweaks' ) . '</p>';
		echo '<textarea class=' . $q . 'large-text code sbtweaks-docs__prompt' . $q . ' rows=' . $q . '5' . $q . ' readonly onclick=' . $q . 'this.select();' . $q . '>' . esc_textarea( $prompt ) . '</textarea>';

		echo '<h3 class=' . $q . 'sbtweaks-docs__heading' . $q . '>' . esc_html__( 'The notes themselves', 'sb-tweaks' ) . '</h3>';

		echo '<form method=' . $q . 'post' . $q . ' action=' . $q . esc_url( admin_url( 'admin-post.php' ) ) . $q . '>';
		echo '<input type=' . $q . 'hidden' . $q . ' name=' . $q . 'action' . $q . ' value=' . $q . 'sbtweaks_save_docs' . $q . '>';
		wp_nonce_field( 'sbtweaks_save_docs' );
		echo '<textarea name=' . $q . 'sbtweaks_docs' . $q . ' rows=' . $q . '18' . $q . ' class=' . $q . 'large-text code sbtweaks-docs' . $q . ' spellcheck=' . $q . 'false' . $q . '>' . esc_textarea( $text ) . '</textarea>';
